...
Command aliases could be supported without being promoted:

- `ls` → `list`
- `rm` → `delete`
- `finish` → `done`

Avoid abbreviating every command; discoverability matters more than shaving two characters. Reconcile command names and remove `unmove` as part of this consistency pass.
